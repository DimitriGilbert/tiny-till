...
test.describe("Storage Quota Edge Cases", () => {
  ...
  test("recovers from quota error by clearing old products", async ({
    page,
  }) => {
    await mockStorageQuota(page, {
      localStorageLimit: 1024,
    ...
    await page.click(
      '[data-testid="product-item-1"] [data-testid="delete-button"]',
    );
  ...
  test("dismisses warning and prevents re-show", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="storage-warning-alert"]'),
    ).not.toBeVisible();
