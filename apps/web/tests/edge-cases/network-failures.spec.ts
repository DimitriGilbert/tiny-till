...
test.describe("Network Failure Edge Cases", () => {
  ...
  test("prevents operations that require network when offline", async ({
    page,
  }) => {
    await setNetworkOffline(page, true);
  ...
  test("persists data across offline-online transition", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="product-card-Persistent Product"]'),
    ).toBeVisible();
  });
  ...
  test("handles connection loss during import", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="product-card-Import During Offline"]'),
    ).toBeVisible();
  });
