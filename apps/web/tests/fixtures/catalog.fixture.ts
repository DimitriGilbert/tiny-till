...
export interface CatalogFixture {
  ...
  addProduct: (page: Page, product: Omit<Product, "id" | "createdAt" | "updatedAt">) => Promise<void>;
  editProduct: (page: Page, productId: string, updates: Partial<Product>) => Promise<void>;
...
export const createCatalogFixture = (): CatalogFixture => {
  return {
    ...
    async addProduct(
      ...
      await page.fill('[data-testid="product-price-input"]', String(product.price));
    ...
    async editProduct(page: Page, productId: string, updates: Partial<Product>): Promise<void> {
      await page.click(`[data-testid="product-item-${productId}"] [data-testid="edit-button"]`);
      ...
      if (updates.price !== undefined) {
        await page.fill('[data-testid="product-price-input"]', String(updates.price));
      }
    ...
    async deleteProduct(page: Page, productId: string): Promise<void> {
      await page.click(`[data-testid="product-item-${productId}"] [data-testid="delete-button"]`);
    ...
    async getProducts(page: Page): Promise<Product[]> {
      const productElements = await page.locator('[data-testid^="product-item-"]').all();
      const products: Product[] = [];

      for (const element of productElements) {
        const name = await element.locator('[data-testid="product-name"]').textContent();
        const priceText = await element.locator('[data-testid="product-price"]').textContent();
        const price = parseFloat((priceText ?? "").replace(/[^0-9.-]+/g, "") || "0");

        products.push({
          id: (await element.getAttribute("data-testid"))?.replace("product-item-", "") || "",
          name: name || "",
          price,
          createdAt: Date.now(),
          updatedAt: Date.now(),
